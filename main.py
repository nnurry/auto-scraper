...
class Scraper:
    ...
    def ingest(self, url, data):
        for wanted_list in data:
            self.scraper.build(url=url, wanted_list=wanted_list)

    @outer
    def get_result(self, url, filename, format=".json"):
        result = self.scraper.get_result_similar(url, grouped=True)
...
scraper.get_result(related["url"], _path('related'))